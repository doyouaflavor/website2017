...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme967
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme967_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme967_html_head_alter( &$head_elements ) {
 ...
 */
function theme967_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme967_preprocess( &$vars, $hook ) {
	$vars['theme967_path'] = base_path() . path_to_theme();
 ...
 */
function theme967_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme967_doctype();
	$vars['rdf'] = _theme967_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme967_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme967_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme967_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme967_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme967_blog_title' );
	}
}
 ...
 */
function theme967_preprocess_page( &$vars ) {
 ...
 */
function theme967_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme967' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme967_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme967_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme967_menu_tree( $vars ) {
 ...
 */
function theme967_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme967_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme967_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme967_breadcrumb_separator' );

	if ( theme_get_setting( 'theme967_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme967_tabs_float() {
	$float = (bool) theme_get_setting( 'theme967_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme967_tabs_node' );
...
function theme967_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme967_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme967_id_safe( $element['#title'] );
 ...
 */
function theme967_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme967_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme967_preprocess_comment( &$variables ) {
...
/* Region options */
function theme967_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme967_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme967_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme967_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme967_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme967_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme967_' . $reg . '_fullwidth' );
	...
	if ( !empty( $block_bg_img ) && ( $block_bg_type == 'image' ) ) {
		$file = file_load( $block_bg_img );
	...
	if ( isset( $bg_video ) && $bg_video && ( $block_bg_type == 'video' ) ) {
		...
		// $content .= '<iframe class="actAsDiv" frameborder="0" height="680" src="http://www.youtube.com/embed/FG-h26AnMHg?autoplay=1&amp;loop=1&amp;playlist=PLXBSvLYHA1OpBjbvY2LDYTqFjn9_xx8t7&amp;showinfo=0&amp;theme=dark&amp;color=white&amp;controls=0&amp;modestbranding=0&amp;start=0&amp;fs=1&amp;iv_load_policy=3&amp;wmode=transparent&amp;rel=0" width="1200"></iframe>';
		// $content .= '<iframe id="myYoutubeVideo" class="myYoutubeVideo" frameborder="0" height="680" src="https://www.youtube.com/embed/FG-h26AnMHg?modestbranding=1&autoplay=1&controls=0&showinfo=0&rel=0&enablejsapi=1&version=3&playerapiid=mbYTP_YTP_1490513145886&origin=http%3A%2F%2Fdev_new.doyouaflavor.tw&allowfullscreen=true&wmode=transparent&iv_load_policy=3&html5=1&widgetid=1&mute=1" width="1200"></iframe>';
...
function theme967_region_suffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme967_' . $reg . '_bg_type' );
	if($block_bg_type == 'none')return;
	$fullwidth = theme_get_setting( 'theme967_' . $reg . '_fullwidth' );
	$block_bg_parallax = theme_get_setting( 'theme967_' . $reg . '_bg_parallax' );
	$block_bg_img = theme_get_setting( 'theme967_' . $reg . '_bg_img' );
...
/* Contact form */
function theme967_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
